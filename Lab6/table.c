...
void *getElements(SET *sp){
    ...
    quickSort( data, 0, sp->count-1, sp->compare);
    return data;
}

//O(n)
void quickSort(void **data, int l, int h, int (*compare)()){
    if(l<h){
        int idx = partition(data, l, h, compare);
        quickSort(data, l, idx-1, compare);
        quickSort(data, idx+1, h, compare);
    }
    return;
}
...
//O(n)
int partition(void **data, int l, int h, int (*compare)()){
    int i = l-1;
    void *piv = data[h];
       for(int j=l; j<h-1; j++){
           if(compare(piv, data[j])>0){
               i++;
               void *temp;
               temp=data[j];
               data[j] = data[i];
